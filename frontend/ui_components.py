...
def display_header():
    ...
    st.set_page_config(
        page_title="AI Agent Chatbot", layout="wide"
    )  # Set page config here
    st.title("🤖 AI Agent Chatbot")
    st.markdown(
        "Ask me anything! I can answer questions using my internal knowledge (RAG) or by searching the web."
    )
    st.markdown("---")
            ...
            if uploaded_file is not None:
                with st.spinner(f"Uploading {uploaded_file.name}..."):
                    try:
                        ...
                        st.success(
                            f"PDF '{upload_data.get('filename')}' uploaded successfully! Processed {upload_data.get('processed_chunks')} pages."
                        )
            ...
            for event in trace_events:
                ...
                if (
                    ...
                    if verdict == "Sufficient":
                        st.success(
                            f"**RAG Verdict:** {verdict} - Relevant info found in Knowledge Base."
                        )
                    else:
                        st.warning(
                            f"**RAG Verdict:** {verdict} - No sufficient info in Knowledge Base. Diverting to Web Search."
                        )

                    if "retrieved_content_summary" in event["details"]:
                        st.markdown(
                            f"**Retrieved Content Summary:** `{event['details']['retrieved_content_summary']}`"
                        )
                elif (
                    ...
                    st.markdown(
                        f"**Web Search Content Summary:** `{event['details']['retrieved_content_summary']}`"
                    )
                elif (
